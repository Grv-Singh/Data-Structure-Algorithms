...
int getSum(int *st, int n, nt qs, int qe){
...
int constructionUtil(int arr[], int ss, int se, int *st, nt si){
    if(ss==se){
        st[si] = arr[ss];
        return arr[ss];
    }
    int mid = getMid(ss,se);
    st[si] = constructionSTUtil(arr,ss,mid,st,si*2+1)+constructionSTUtil(arr,mid+1,se,st,si*2+2);
    return st[si];
}
